...
#endif
template<class T> SGF_INLINE_FORCED int	MaxIndex( T x, T y ) { return  ( x > y ) ? 0 : 1; }
template<class T> SGF_INLINE_FORCED int	MinIndex( T x, T y ) { return ( x < y ) ? 0 : 1; }

template<class T> SGF_INLINE_FORCED T	Max3( T x, T y, T z ) { return ( x > y ) ? ( ( x > z ) ? x : z ) : ( ( y > z ) ? y : z ); }
template<class T> SGF_INLINE_FORCED T	Min3( T x, T y, T z ) { return ( x < y ) ? ( ( x < z ) ? x : z ) : ( ( y < z ) ? y : z ); }
template<class T> SGF_INLINE_FORCED int	Max3Index( T x, T y, T z ) { return ( x > y ) ? ( ( x > z ) ? 0 : 2 ) : ( ( y > z ) ? 1 : 2 ); }
template<class T> SGF_INLINE_FORCED int	Min3Index( T x, T y, T z ) { return ( x < y ) ? ( ( x < z ) ? 0 : 2 ) : ( ( y < z ) ? 1 : 2 ); }

template<class T> SGF_INLINE_FORCED T	Sign( T f ) { return ( f > 0 ) ? 1 : ( ( f < 0 ) ? -1 : 0 ); }
template<class T> SGF_INLINE_FORCED T	Square( T x ) { return x * x; }
template<class T> SGF_INLINE_FORCED T	Cube( T x ) { return x * x * x; }
...
private:
	...
	static SGF_Dword			iSqrt[SQRT_TABLE_SIZE];
	static bool					initialized;
};

SGF_INLINE_FORCED float CMath::RSqrt( float x ) {
...
SGF_INLINE_FORCED float CMath::InvSqrt16( float x ) {

	SGF_Dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
SGF_INLINE_FORCED float CMath::InvSqrt( float x ) {
...
SGF_INLINE_FORCED double CMath::InvSqrt64( float x ) {
	SGF_Dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
SGF_INLINE_FORCED float CMath::Sqrt16( float x ) {
	return x * InvSqrt16( x );
}

SGF_INLINE_FORCED float CMath::Sqrt( float x ) {
	return x * InvSqrt( x );
}

SGF_INLINE_FORCED double CMath::Sqrt64( float x ) {
	return x * InvSqrt64( x );
}

SGF_INLINE_FORCED float CMath::Sin( float a ) {
	return sinf( a );
}

SGF_INLINE_FORCED float CMath::Sin16( float a ) {
	float s;
...
SGF_INLINE_FORCED double CMath::Sin64( float a ) {
	return sin( a );
}

SGF_INLINE_FORCED float CMath::Cos( float a ) {
	return cosf( a );
}

SGF_INLINE_FORCED float CMath::Cos16( float a ) {
	float s, d;
...
SGF_INLINE_FORCED double CMath::Cos64( float a ) {
	return cos( a );
}

SGF_INLINE_FORCED void CMath::SinCos( float a, float &s, float &c ) {
#if defined(_MSC_VER)
	_asm {
		fld		a
		fsincos
...
SGF_INLINE_FORCED void CMath::SinCos16( float a, float &s, float &c ) {
	float t, d;
...
SGF_INLINE_FORCED void CMath::SinCos64( float a, double &s, double &c ) {
#if defined(_MSC_VER)
	_asm {
		fld		a
		fsincos
...
SGF_INLINE_FORCED float CMath::Tan( float a ) {
	return tanf( a );
}

SGF_INLINE_FORCED float CMath::Tan16( float a ) {
	float s;
	bool reciprocal;
...
SGF_INLINE_FORCED double CMath::Tan64( float a ) {
	return tan( a );
}

SGF_INLINE_FORCED float CMath::ASin( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
SGF_INLINE_FORCED float CMath::ASin16( float a ) {
...
SGF_INLINE_FORCED double CMath::ASin64( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
SGF_INLINE_FORCED float CMath::ACos( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
SGF_INLINE_FORCED float CMath::ACos16( float a ) {
	if ( FLOATSIGNBITSET( a ) ) {
		if ( a <= -1.0f ) {
			return PI;
		}
...
SGF_INLINE_FORCED double CMath::ACos64( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
SGF_INLINE_FORCED float CMath::ATan( float a ) {
	return atanf( a );
}

SGF_INLINE_FORCED float CMath::ATan16( float a ) {
	float s;
...
SGF_INLINE_FORCED double CMath::ATan64( float a ) {
	return atan( a );
}

SGF_INLINE_FORCED float CMath::ATan( float y, float x ) {
	return atan2f( y, x );
}

SGF_INLINE_FORCED float CMath::ATan16( float y, float x ) {
	float a, s;
...
SGF_INLINE_FORCED double CMath::ATan64( float y, float x ) {
	return atan2( y, x );
}

SGF_INLINE_FORCED float CMath::Pow( float x, float y ) {
	return powf( x, y );
}

SGF_INLINE_FORCED float CMath::Pow16( float x, float y ) {
	return Exp16( y * Log16( x ) );
}

SGF_INLINE_FORCED double CMath::Pow64( float x, float y ) {
	return pow( x, y );
}

SGF_INLINE_FORCED float CMath::Exp( float f ) {
	return expf( f );
}

SGF_INLINE_FORCED float CMath::Exp16( float f ) {
...
SGF_INLINE_FORCED double CMath::Exp64( float f ) {
	return exp( f );
}

SGF_INLINE_FORCED float CMath::Log( float f ) {
	return logf( f );
}

SGF_INLINE_FORCED float CMath::Log16( float f ) {
	int i, exponent;
	float y, y2;
...
SGF_INLINE_FORCED double CMath::Log64( float f ) {
	return log( f );
}

SGF_INLINE_FORCED int CMath::IPow( int x, int y ) {
	int r; for( r = x; y > 1; y-- ) { r *= x; } return r;
}

SGF_INLINE_FORCED int CMath::ILog2( float f ) {
	return ( ( (*reinterpret_cast<int *>(&f)) >> IEEE_FLT_MANTISSA_BITS ) & ( ( 1 << IEEE_FLT_EXPONENT_BITS ) - 1 ) ) - IEEE_FLT_EXPONENT_BIAS;
}

SGF_INLINE_FORCED int CMath::ILog2( int i ) {
	return ILog2( (float)i );
}

SGF_INLINE_FORCED int CMath::BitsForFloat( float f ) {
	return ILog2( f ) + 1;
}

SGF_INLINE_FORCED int CMath::BitsForInteger( int i ) {
	return ILog2( (float)i ) + 1;
}

SGF_INLINE_FORCED int CMath::MaskForFloatSign( float f ) {
	return ( (*reinterpret_cast<int *>(&f)) >> 31 );
}

SGF_INLINE_FORCED int CMath::MaskForIntegerSign( int i ) {
	return ( i >> 31 );
}

SGF_INLINE_FORCED int CMath::FloorPowerOfTwo( int x ) {
	return CeilPowerOfTwo( x ) >> 1;
}

SGF_INLINE_FORCED int CMath::CeilPowerOfTwo( int x ) {
	x--;
	x |= x >> 1;
	x |= x >> 2;
	x |= x >> 4;
...
SGF_INLINE_FORCED bool CMath::IsPowerOfTwo( int x ) {
	return ( x & ( x - 1 ) ) == 0 && x > 0;
}

SGF_INLINE_FORCED int CMath::BitCount( int x ) {
	x -= ( ( x >> 1 ) & 0x55555555 );
...
SGF_INLINE_FORCED int CMath::BitReverse( int x ) {
...
SGF_INLINE_FORCED int CMath::Abs( int x ) {
   int y = x >> 31;
   return ( ( x ^ y ) - y );
}

SGF_INLINE_FORCED float CMath::Fabs( float f ) {
	int tmp = *reinterpret_cast<int *>( &f );
	tmp &= 0x7FFFFFFF;
	return *reinterpret_cast<float *>( &tmp );
}

SGF_INLINE_FORCED float CMath::Floor( float f ) {
	return floorf( f );
}

SGF_INLINE_FORCED float CMath::Ceil( float f ) {
	return ceilf( f );
}

SGF_INLINE_FORCED float CMath::Rint( float f ) {
	return floorf( f + 0.5f );
}

SGF_INLINE_FORCED int CMath::Ftoi( float f ) {
	return (int) f;
}

SGF_INLINE_FORCED int CMath::FtoiFast( float f ) {
#if defined(_MSC_VER)
	int i;
	__asm fld		f
...
SGF_INLINE_FORCED unsigned long CMath::Ftol( float f ) {
	return (unsigned long) f;
}

SGF_INLINE_FORCED unsigned long CMath::FtolFast( float f ) {
#if defined(_MSC_VER)
...
SGF_INLINE_FORCED signed char CMath::ClampChar( int i ) {
	if ( i < -128 ) {
		return -128;
	}
	if ( i > 127 ) {
...
SGF_INLINE_FORCED signed short CMath::ClampShort( int i ) {
	if ( i < -32768 ) {
		return -32768;
	}
	if ( i > 32767 ) {
...
SGF_INLINE_FORCED int CMath::ClampInt( int min, int max, int value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
SGF_INLINE_FORCED float CMath::ClampFloat( float min, float max, float value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
SGF_INLINE_FORCED float CMath::AngleNormalize360( float angle ) {
...
SGF_INLINE_FORCED float CMath::AngleNormalize180( float angle ) {
...
SGF_INLINE_FORCED float CMath::AngleDelta( float angle1, float angle2 ) {
	return AngleNormalize180( angle1 - angle2 );
}

SGF_INLINE_FORCED int CMath::FloatHash( const float *array, const int numFloats ) {
	int i, hash = 0;
	const int *ptr;
