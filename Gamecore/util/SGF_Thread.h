...
ThreadId_t *_my_ThreadId_t;
	...
	/** cria um novo Trhead
		\param function: A fun��o (assinatura ThreadFunction) que ira executar no thread criado
		\param arg: ponteiro para os argumentos que ser�o passados para a fun��o function
	...
	/**  A thread pode executar o thread join e aguardar at� a outra thread terminar.
	     Em geral o thread join � utilizado para a thread-m�e se sincronizar com uma das threads-filho.
...
public:
    /** constr�i o objeto, mas n�o inicia o novo thread ainda
	**/
    CRegisteredThread();

    /** constr�i o objeto, e j� inicia o novo thread
	\param thread: a fun��o que ser� executada pelo novo thread
	\param arg: os par�metros que ser�o passados para fun��o a ser executada
	**/
   CRegisteredThread(Thread::ThreadFunction thread, void * arg);

    /** \brief inicia um thread
	\param thread: o thread a ser iniciado
	\param arg: ospar�metros que ser�o passados para a fun��o a ser executada
	**/
	...
	**/
    bool isRunning();
	/** \brief espera at� que o thread termine
	**/
    void kill();
 ...
 *
 * isEmpty()
 * \brief Verifica se h� ou n�o threads na fila
 * \return: true se n�o h� nenhum elemento na fila de threads ou
...
public:
	...
	/** retorna o valor da vari�vel boolean mas antes faz um lock nela
	\return: valor (verdadeiro ou falso) que estava armazenado
	**/
    bool get();
	/** seta o valor da vari�vel boolean mas antes faz um lock nela
...
protected:
    ...
    static void * runit(void * arg){
        ...
        try{
            ...
            me->exception = new Exception::CBase(base);
        } catch (...){
            me->exception = new Exception::CBase(__FILE__, __LINE__);
        }
        me->future.acquire();
        me->done = true;
        me->future.release();
