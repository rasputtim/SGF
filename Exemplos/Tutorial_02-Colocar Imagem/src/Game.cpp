...
void Game::draw(SGF::CBitmap * work ) {
	...
	// DRAW SCORE + INFO
	SGF::Debug::debug(SGF::Debug::gameEngine,__FUNCTION__) <<"WILL SHOW OUR BITMAP" <<endl;
	if( bitmaptoShow) bitmaptoShow->Blit(200,200);
...
void Game::gameInit() {
    std::string tmpstr;
		...
		bitmaptoShow = new SGF::CBitmap("icon.bmp");
		bitmaptoShow->applyDefTrans();
...
Game::~Game()
{
	...
	if (bitmaptoShow) delete bitmaptoShow;
}
