...
#include "geometry/SGF_TraceModel.h"
...
============
*/
void idTraceModel::SetupBox( const Gui::CBoundBox &boxBounds ) {
	int i;
...
============
*/
void idTraceModel::SetupOctahedron( const Gui::CBoundBox &octBounds ) {
...
============
*/
void idTraceModel::SetupDodecahedron( const Gui::CBoundBox &dodBounds ) {
...
============
*/
void idTraceModel::SetupCylinder( const Gui::CBoundBox &cylBounds, const int numSides ) {
	int i, n, ii, n2;
	float angle;
	CVector3D halfSize;
	...
	if ( n * 2 > MAX_TRACEMODEL_VERTS ) {
		Debug::debug(Debug::error,__FUNCTION__)<<"WARNING:  too many vertices" <<endl;
		n = MAX_TRACEMODEL_VERTS / 2;
	}
	if ( n * 3 > MAX_TRACEMODEL_EDGES ) {
		Debug::debug(Debug::error,__FUNCTION__)<< "WARNING:  too many sides" <<endl;
		n = MAX_TRACEMODEL_EDGES / 3;
	}
	if ( n + 2 > MAX_TRACEMODEL_POLYS ) {
		Debug::debug(Debug::error,__FUNCTION__)<< "WARNING:  too many polygons" <<endl;
...
============
*/
void idTraceModel::SetupCone( const Gui::CBoundBox &coneBounds, const int numSides ) {
	int i, n, ii;
	float angle;
	CVector3D halfSize;
	...
	if ( n + 1 > MAX_TRACEMODEL_VERTS ) {
		Debug::debug(Debug::error,__FUNCTION__)<< "WARNING:  too many vertices\n"<<endl;
		n = MAX_TRACEMODEL_VERTS - 1;
	}
	if ( n * 2 > MAX_TRACEMODEL_EDGES ) {
		Debug::debug(Debug::error,__FUNCTION__)<< "WARNING:  too many edges\n"<<endl;
		n = MAX_TRACEMODEL_EDGES / 2;
	}
	if ( n + 1 > MAX_TRACEMODEL_POLYS ) {
		Debug::debug(Debug::error,__FUNCTION__)<< "WARNING:  too many polygons\n"<<endl;
...
============
*/
void idTraceModel::SetupPolygon( const CVector3D *v, const int count ) {
	int i, j;
	CVector3D mid;
...
void idTraceModel::SetupPolygon( const idWinding &w ) {
	...
	verts = (CVector3D *) _alloca16( w.GetNumPoints() * sizeof( CVector3D ) );
...
============
*/
void idTraceModel::Translate( const CVector3D &translation ) {
	int i;
...
============
*/
void idTraceModel::Rotate( const CMatriz3D &rotation ) {
	int i, j, edgeNum;
...
============
*/
int idTraceModel::GetProjectionSilhouetteEdges( const CVector3D &projectionOrigin, int silEdges[MAX_TRACEMODEL_EDGES] ) const {
	int i, j, edgeNum;
...
============
*/
int idTraceModel::GetParallelProjectionSilhouetteEdges( const CVector3D &projectionDir, int silEdges[MAX_TRACEMODEL_EDGES] ) const {
	int i, j, edgeNum;
	int edgeIsSilEdge[MAX_TRACEMODEL_EDGES+1];
	const traceModelPoly_t *poly;
...
void idTraceModel::VolumeIntegrals( struct volumeIntegrals_s &integrals ) const {
	...
	for ( i = 0; i < numPolys; i++ ) {
		...
		nx = CMath::Fabs( poly->normal[0] );
		ny = CMath::Fabs( poly->normal[1] );
		nz = CMath::Fabs( poly->normal[2] );
		if ( nx > ny && nx > nz ) {
			c = 0;
		}
		else {
...
============
*/
void idTraceModel::GetMassProperties( const float density, float &mass, CVector3D &centerOfMass, CMatriz3D &inertiaTensor ) const {
