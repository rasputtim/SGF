...
@param SFFBEHAVIOR *behavior: estrutura contendo uma s�rie de vari�veis que controlam o comportamento do objeto criado. Se for NULL utiliza o comportamento padr�o
@return construtores n�o retornam
...
my_act_path="";
	...
	if( extract_header() ) { //preenche _myheader antes de alocar mem�rias
	...
	// Cria uma tabela para checar se a imagem lincada est� ligada a imagens numeriacamente duplicadas
...
@brief ajusta  as vari�veis de comportamento da classe
@param BEHAVIOR *behavior: estrutura contendo uma s�rie de vari�veis que controlam o comportamento do objeto criado. Se for NULL utiliza o comportamento padr�o
@return nada
@warning
**/
...
@brief extrai o cabe�alho do arquivo ssf para a extrutura header
...
int CSFFManager::extract_header() {
    ...
    //! checa se a assinatura do arquivo est� certa
...
@brief Extrai as imagens PCX do arquivo SFF de acordo com o comportamento definido na constru��o do objeto.
@return 1 se falhar e 0 se tiver sucesso
@warning
**/
int CSFFManager::extract() {
    ...
    // o cabe�alho j� foi extra�do pelo construtor da classe
	...
	// Preenche as tabelas de offset e palete para a condi��o linux_character
    ...
    for( i = 0 ; i < my_header.nb_imgs ; i++ ) {
        ...
        // ser� deletada atrav�s da dele��o do CMUgenSprite???
       ...
       /* This might often not be sufficient, but is it necessary?
        ...
        //determina o tamanho para o �ltimko subfile
        ...
        if( sub.length == 0 ){  // a imagem � linkada
          is_linked = 1;
		...
		if (is_linked) {
			/* ainda n�o sei o que fazer aqui....
			//deve achar a imagem na lista pelo nome, em caso dela n�o ter sido criada no siatema de arquivo...????
		...
		}else { // n�o � uma imagem linkada
		if( is_8bitpal ){ // na primeira imagem ele entra aqui. isso retira a necessidade dessa parte na fase de cria��o do PCX
                  ...
                  if( !(sub.group==9000 && sub.img==1 && (!sub.same_pal || is_8bitpal==-1)) || is_8bitpal==2 )
                      {
						...
						// copiar o pallete para o final da posi��o de mem�ria imagePCX
						...
						//atualiza o tamanho do bloco de mem�ria
		...
		} // fim da adi��o do pallete na Lista ligada
		...
		if( my_create_pcx ){
          ...
          /* I'll explain the logic used here later, if I'm feeling ambitious. =P  -Winane */
			// copia o palette da 1a imagem, o qual ser� utilizado nas imagens posteriores que utilizam o palette compartilhado
		  if( is_8bitpal ){
              if( !found_1st && !my_use_act && is_8bitpal!=2 ){
                  memcpy( palsaveD, pcxdata+reallength-768, 768 );
                  memcpy( palsave1, palsaveD, 768 );
    ...
    /* Output to stdout. */
	 ...
	 //Adiciona o Arquivo PCX � lista ligada
...
@brief verifica as quest�es do sistema de arquivos
@return 1 se falhar e 0 se tiver sucesso
@warning
**/
