...
public:
	virtual bool	Solve( const CMatrizXD &o_m, CVectorXD &o_x, const CVectorXD &o_b, const CVectorXD &o_lo, const CVectorXD &o_hi, const int *o_boxIndex );

private:
	CMatrizXD			m;					// original matrix
	CVectorXD			b;					// right hand side
	CVectorXD			lo, hi;				// low and high bounds
	CVectorXD			f, a;				// force and acceleration
	CVectorXD			delta_f, delta_a;	// delta force and delta acceleration
	CMatrizXD			clamped;			// LU factored sub matrix for clamped variables
	CVectorXD			diagonal;			// reciprocal of diagonal of U of the LU factored sub matrix for clamped variables
...
private:
	bool			FactorClamped();
	void			SolveClamped( CVectorXD &x, const float *b );
...
============
*/
void CLinCP_Square::SolveClamped( CVectorXD &x, const float *b ) {
	int i, j;
	float sum;
...
============
*/
SGF_INLINE_FORCED void CLinCP_Square::CalcForceDelta( int d, float dir ) {
	int i;
	float *ptr;
...
============
*/
SGF_INLINE_FORCED void CLinCP_Square::CalcAccelDelta( int d ) {
	int j;
	float dot;
...
============
*/
SGF_INLINE_FORCED void CLinCP_Square::ChangeForce( int d, float step ) {
...
============
*/
SGF_INLINE_FORCED void CLinCP_Square::ChangeAccel( int d, float step ) {
...
============
*/
bool CLinCP_Square::Solve( const CMatrizXD &o_m, CVectorXD &o_x, const CVectorXD &o_b, const CVectorXD &o_lo, const CVectorXD &o_hi, const int *o_boxIndex ) {
	...
	numUnbounded = 0;
	for ( i = 0; i < m.GetNumRows(); i++ ) {
		if ( lo[i] == -CMath::INFINITY && hi[i] == CMath::INFINITY ) {
			if ( numUnbounded != i ) {
				Swap( numUnbounded, i );
			}
			numUnbounded++;
	...
	if ( boxIndex ) {
		for ( i = m.GetNumRows() - 1; i >= numUnbounded; i-- ) {
			if ( boxIndex[i] >= 0 && ( lo[i] != -CMath::INFINITY || hi[i] != CMath::INFINITY ) ) {
				boxStartIndex--;
				if ( boxStartIndex != i ) {
	...
	for ( i = numUnbounded; i < m.GetNumRows(); i++ ) {
		...
		if ( i == boxStartIndex ) {
			...
			for ( j = boxStartIndex; j < m.GetNumRows(); j++ ) {
				s = o_x[boxIndex[j]];
				if ( lo[j] != -CMath::INFINITY ) {
					lo[j] = - CMath::Fabs( lo[j] * s );
				}
				if ( hi[j] != CMath::INFINITY ) {
	...
	if ( numIgnored ) {
		if ( lcp_showFailures.GetBool() ) {
			Debug::debug(Debug::math,__FUNCTION__)<< "idLCP_Symmetric::Solve: "<< numIgnored <<" of " << m.GetNumRows() - numUnbounded<<" bounded variables ignored\n" <<endl
		}
	}
#endif
...
class idLCP_Symmetric : public CLinCP {
public:
	virtual bool	Solve( const CMatrizXD &o_m, CVectorXD &o_x, const CVectorXD &o_b, const CVectorXD &o_lo, const CVectorXD &o_hi, const int *o_boxIndex );
...
private:
	...
	CVectorXD			f, a;				// force and acceleration
	CVectorXD			delta_f, delta_a;	// delta force and delta acceleration
	CMatrizXD			clamped;			// LDLt factored sub matrix for clamped variables
	CVectorXD			diagonal;			// reciprocal of diagonal of LDLt factored sub matrix for clamped variables
	CVectorXD			solveCache1;		// intermediate result cached in SolveClamped
	CVectorXD			solveCache2;		// "
	int				numUnbounded;		// number of unbounded variables
	int				numClamped;			// number of clamped variables
...
============
*/
void idLCP_Symmetric::SolveClamped( CVectorXD &x, const float *b ) {
...
void idLCP_Symmetric::AddClamped( int r, bool useSolveCache ) {
	...
	if ( d == 0.0f ) {
		Debug::debug(Debug::math,__FUNCTION__)<< "idLCP_Symmetric::AddClamped: updating factorization failed\n" <<endl;
		numClamped++;
		return;
	}
...
void idLCP_Symmetric::RemoveClamped( int r ) {
	...
	if ( r == 0 ) {
		...
		if ( numClamped == 1 ) {
			diag = rowPtrs[0][0];
			if ( diag == 0.0f ) {
				Debug::debug(Debug::math,__FUNCTION__)<< "idLCP_Symmetric::RemoveClamped: updating factorization failed\n" <<endl;
				return;
			}
			clamped[0][0] = diag;
...
============
*/
SGF_INLINE_FORCED void idLCP_Symmetric::CalcForceDelta( int d, float dir ) {
	int i;
	float *ptr;
...
============
*/
SGF_INLINE_FORCED void idLCP_Symmetric::CalcAccelDelta( int d ) {
	int j;
	float dot;
...
============
*/
SGF_INLINE_FORCED void idLCP_Symmetric::ChangeForce( int d, float step ) {
	// only the clamped variables and current variable have a force delta unequal zero
	SIMDProcessor->MulAdd( f.ToFloatPtr(), step, delta_f.ToFloatPtr(), numClamped );
	f[d] += step * delta_f[d];
}
...
============
*/
SGF_INLINE_FORCED void idLCP_Symmetric::ChangeAccel( int d, float step ) {
	// only the not clamped variables, including the current variable, can have an acceleration unequal zero
	SIMDProcessor->MulAdd( a.ToFloatPtr() + numClamped, step, delta_a.ToFloatPtr() + numClamped, d - numClamped + 1 );
}
...
============
*/
bool idLCP_Symmetric::Solve( const CMatrizXD &o_m, CVectorXD &o_x, const CVectorXD &o_b, const CVectorXD &o_lo, const CVectorXD &o_hi, const int *o_boxIndex ) {
	int i, j, n, limit, limitSide, boxStartIndex;
	float dir, maxStep, dot, s;
	char *failed;
	...
	if ( numUnbounded ) {
		...
		// factor and solve for unbounded variables
		if ( !FactorClamped() ) {
			Debug::debug(Debug::math,__FUNCTION__)<< "idLCP_Symmetric::Solve: unbounded factorization failed\n" <<endl;
			return false;
		}
		SolveClamped( f, b.ToFloatPtr() );
	...
	if ( numIgnored ) {
		if ( lcp_showFailures.GetBool() ) {
			Debug::debug(Debug::math,__FUNCTION__)<< "idLCP_Symmetric::Solve: "<< numIgnored<< " of "<< m.GetNumRows() - numUnbounded<<" bounded variables ignored"  <<endl;
		}
	}
#endif
	...
	if ( failed ) {
		//s if ( lcp_showFailures.GetBool() ) {
			Debug::debug(Debug::math,__FUNCTION__)<< "idLCP_Symmetric::Solve: "<< failed<< " ( "<< m.GetNumRows() - i<< " of "<<  m.GetNumRows() - numUnbounded<<" bounded variables ignored)"<<endl;
		//}
		for ( j = i; j < m.GetNumRows(); j++ ) {
			f[j] = 0.0f;
		}
...
CLinCP *CLinCP::AllocSymmetric() {
	CLinCP *lcp = new idLCP_Symmetric;
	lcp->SetMaxIterations( 32 );
	return lcp;
}
