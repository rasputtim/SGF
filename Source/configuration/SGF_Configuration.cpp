...
// INICIALIZA��O DAS VAI�VEIS EST�TICAS DE CONFIGURA��O GLOBAL
...
CEngineSysVar CGlobalConfiguration::currentSGFDir ("currentSGFDir",SGF_CONFIG_FILE_DIR,SYSVAR_ENGINE,"configura o diret�rio onde est�o os arquivos de configuracao do engine ");
...
/** ao criar o estado inicial do SDl que ser� uilizdo para fazer as inicializa��es
ele utiliza o tamanho da janela default, configurado nas vari�veis screenWidth e screenHeight
estes valores devem ser modificados posteriormente antes da inicializa��o das janelas
**/
...
//=============CALL BACK SYSTEM=======================
/** 
\brief M�todo que o nome do arquivo de configura��o especial definido para o game usu�rio da Engine
\param fileName: nome do arquivo a ser armazenado
\note este arquivo ser� carregado depois pelo m�todo loadUserConfigurations()
**/	
...
/** 
\brief M�todo que retorna o caminho do arquivo de configura��o especial definido para o game usu�rio da Engine no m�doto setUserConfigFile
\note Se o arquivo n�o existir....
**/	
...
/** m�todo usado par registrar par�metros que ser�o
encontrados nos arquivos de configura��o XML de usu�rios  do SGF Fabric
registrados pelo m�todo setUserConfigFile(filename)
\param string head: head XML  do par�metro a ser registrada
\param string option: option XML do par�metro a ser registrado
\param string parameter: nome do par�metro a ser registrado
...
void CGlobalConfiguration::registerConfigValue(string head, string option, string parameter){
	CConfiguration::confHead::iterator first_it = userProperties.find(head);
            if (first_it == userProperties.end()) { //nao achou o head. adicionar� um
...
string CGlobalConfiguration::findConfigValue(string head, string option, string parameter){
	confHead::iterator first_it = userProperties.find(head);
			...
			{ //achou o head. agora achar� o option
				confOption::iterator second_it = first_it->second.find(option);
				if (second_it != first_it->second.end())
				{//achou o option. agora achar� o parameter
					confParameter::iterator it = second_it->second.find(option);
					if (it != second_it->second.end())
					{ //achou oparameter. agora achar� o valor
					return it->second;
...
/** 
\brief M�todo que carrega as configura��es definidas pelo programa usu�rio da Engine SGFFabric
       cada programa usu�rio pode definir arquivos XML com configura��o. Este m�todo carrega esses arquivos
\param file: nome do arquivo a carregar
\note este m�todo preserva o nome do arquivo j� cadast
      na vari�vel userConfigFileName.
\note antes de carregar o arquivo, deve se registrar as heads, options e parameters que ser�o
      encontrados nesse arquivo atrav�s do m�todo registerConfigValue
**/
...
\brief M�todo que carrega as configura��es definidas pelo programa usu�rio da Engine SGFFabric
       cada programa usu�rio pode definir arquivos XML com configura��o. Este m�todo carrega esses arquivos
\note carrega o arquivo registrado na vari�vel userConfigFileName.
\note antes de carregar o arquivo, deve se registrar as heads, options e parameters que ser�o
      encontrados nesse arquivo atrav�s do m�todo registerConfigValue
**/
void CGlobalConfiguration::loadUserConfigurations(){
	//! n�o pode por mensagens de debug neste m�todo. n�o sei porque ainda????
				...
				first_it != userProperties.end(); first_it++) {
					...
					if (*headView == first_it->first) {
						...
						second_it !=  first_it->second.end(); second_it++) {  // para cada option
							...
							if (*head == second_it->first) { // se o token � o nome da op��o
								...
								while (secondHead.hasMore()){
									//string optionName;
									//optionName =*secondHead ;//= nome do par�metro
									...
									if (customervar !=NULL) {
										string valuestr;
											...
											//n�o � necess�rio visto que o par�metro j� � o valor da Sysvar
...
// Cria um Mapa Global indexado por n�mero, com ponteiros para inst�ncias da classe CConfiguration
// indice 0 = configura��es do player 1
// indice 1 = configura��es do player 2
...
// Objetivo: define as teclas default para esta inst�ncia de CConfiguration 
// retorna: o objeto 
...
// Objetivo: Cria uma inst�ncia da Classe CConfiguration e define as teclas default para o Player1
...
// Objetivo: Cria uma inst�ncia da Classe CConfiguration e define as teclas default para o Player2
// retorna: o objeto criado
...
/**
\brief: Cria um novo objeto do tipo CConfiguration e define as teclas padr�o para este objeto, caso seja o player 1 ou 2
// Par�metro: set=> que define se � o set=0 para o player 1, ou set=1 para o player 2
// Todo: espandir esse m�todo para configurar teclas padr�o para outros tipos de objeto, recebendo como par�metro o tipo do objeto tamb�m
**/
...
// Par�metro: outro objeto da classe CConfiguration

CConfiguration::CConfiguration( const CConfiguration & config ):  //passagem por refer�ncia
...
int CConfiguration::getJoystickKey(CInput::GameKeys which, int facing, int vFacing) const {
   ...
   return CJoystick::Invalid; // s� para evitar warning do compilador
}
...
int CConfiguration::getMouseKey( CInput::GameKeys which, int facing , int vFacing) const {
	...
	return CMouse::Invalid; // s� para evitar warning do compilador
}
...
Filesystem::CAbsolutePath CConfiguration::getEngineConfigFile(){
	;
		//! TODO   M�todo de find para o arquivo de configura��o
...
void CConfiguration::loadEngineConfiguration() throw (CLoadException) {
	...
	if (!ConfigFile.exist()) throw CLoadException(__FILE__, __LINE__, string("Config file  Does Not Exist - Using Default Configuration Par�meters" ));
