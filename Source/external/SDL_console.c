...
#include "external/SDL_console.h"
...
#ifdef HAVE_SDLIMAGE
#include "../../ExternalLibs/SDL_image-1.2.10/SDL_image.h"
#endif
...
void CON_Show(ConsoleInformation *console) {
	if(console) {
		...
		console->WasUnicode = 0;//SDL_EnableUNICODE(-1);
		//SDL_EnableUNICODE(1);
	}
}
...
int CON_Background(ConsoleInformation *console, const char *image, int x, int y) {
	...
	console->BackgroundImage = temp;//SDL_DisplayFormat(temp);
	SDL_FreeSurface(temp);
	console->BackX = x;
	console->BackY = y;
...
int CON_Resize(ConsoleInformation *console, SDL_Rect rect) {
	...
	console->ConsoleSurface = Temp;//SDL_DisplayFormat(Temp);
	SDL_FreeSurface(Temp);
	...
	console->InputBackground = Temp;//SDL_DisplayFormat(Temp);
	SDL_FreeSurface(Temp);
...
void CON_SetPrompt(ConsoleInformation *console, char* newprompt) {
	...
	if(strlen(newprompt) < console->VChars)
		console->Prompt = my_strdup(newprompt);
	else
...
void CON_TabCompletion(ConsoleInformation *console) {
	...
	command = my_strdup(console->LCommand);
...
void Cursor_Add(ConsoleInformation *console, SDL_Event *event) {
	...
	if(strlen(Topmost->Command) + 1 < CON_CHARS_PER_LINE && event->key.keysym.sym ){ //unicode) {
		Topmost->CursorPos++;
		len = strlen(Topmost->LCommand);
		Topmost->LCommand[len] = (char)event->key.keysym.sym;//.unicode;
