...
void scale2x(SDL_Surface *src, SDL_Surface *dst)
{
     //if(SDL_LockSurface(src) < 0)
        //Debug::debug(Debug::resources,__FUNCTION__) << "scale2x:Was not able to lock work surface");
    
   // if(SDL_LockSurface(dst) < 0)
        //Debug::debug(Debug::resources,__FUNCTION__) << "scale2x:Was not able to lock screen surface");
